...
import { ragContextSchema, type RagContext, type Scope } from "./contracts.js";
...
function validateResponse(value: unknown, request: KnowledgeQuery): RagContext {
  ...
  if (
    ...
    context.evidence.some((item, index) =>
      item.rank !== index + 1 || !request.scopes.includes(item.scope)
    )
  ) {
...
export function queryKnowledgeBase(request: KnowledgeQuery): Promise<RagContext> {
