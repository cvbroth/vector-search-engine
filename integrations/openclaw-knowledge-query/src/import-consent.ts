...
export function parseExplicitImportConsent(content: unknown): AccessKind | null {
  if (typeof content !== "string" || content.length > 200 || /[\r\n]/u.test(content)) return null;
  const text = content.normalize("NFKC").replace(/\s+/gu, "").toLowerCase();
  if (PRIVATE_CHINESE.test(text)) return "private";
  if (SHARED_CHINESE.test(text)) return "shared";
  if (/^(?:please)?(?:import|save|add)(?:this|the)?(?:attachment|file|document|pdf)?(?:to|into)(?:my)?privateknowledgebase[.!]?$/u.test(text)) return "private";
  if (/^(?:please)?(?:import|save|add)(?:this|the)?(?:attachment|file|document|pdf)?(?:to|into)(?:householdshared|familyshared|family|shared)knowledgebase[.!]?$/u.test(text)) return "shared";
  return null;
}
